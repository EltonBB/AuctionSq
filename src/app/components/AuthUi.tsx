...
export function AuthShell({ children }: AuthShellProps) {
  return (
    <main className="brand-surface relative min-h-[100dvh] overflow-hidden text-[#352B24] lg:grid lg:grid-cols-[minmax(0,0.95fr)_minmax(520px,1fr)]">
      <Link
        href="/"
        className="absolute left-4 top-4 z-30 inline-flex items-center gap-2 rounded-full bg-white/86 px-4 py-2 text-xs font-black text-[#5e4c3f] shadow-[0_12px_32px_rgba(53,43,36,0.10)] ring-1 ring-[#f0d9c4] transition hover:text-[#D96C2D] active:translate-y-px lg:left-8 lg:top-8 lg:bg-[#352B24]/78 lg:text-white lg:ring-white/10 lg:hover:text-[#F7D8B5]"
      >
...
function AuthVisualPanel() {
  return (
    <aside className="relative hidden min-h-[100dvh] overflow-hidden bg-[#130d0a] lg:block">
      ...
      <div className="absolute bottom-10 left-8 max-w-[440px] text-white xl:bottom-14 xl:left-12">
        <div className="inline-flex rounded-2xl bg-white/94 px-4 py-3 shadow-[0_18px_50px_rgba(0,0,0,0.28)]">
          <BrandLogo />
        </div>
        <p className="mt-6 text-3xl font-black leading-tight text-white xl:text-4xl">
          Eksperienca e ankandit fillon ketu.
        </p>
        <p className="mt-3 max-w-sm text-sm font-medium leading-6 text-[#f8dfc8]">
