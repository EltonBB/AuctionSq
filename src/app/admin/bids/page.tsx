  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-5">
        ...
        <p className="mt-1 text-sm text-slate-500">Monitoro historikun dhe anulo ofertat e dyshimta me arsyetim.</p>
      </div>
                ...
                <td>
                  {bid.status === "active" ? (
                    <form action={cancel} className="flex gap-2">
                      ...
                      <input name="reason" placeholder="Arsyeja e anulimit" className="w-44 rounded-lg border border-slate-200 px-3 py-2 text-xs" />
                      <ConfirmSubmitButton
                        ...
                        confirmMessage="Anulo këtë ofertë?"
                      >
                        Anulo
                      </ConfirmSubmitButton>
                    </form>
                  ) : (
                    <span className="text-xs text-slate-400">{bid.cancelled_reason || "Anuluar"}</span>
                  )}
                </td>
              </tr>
            ))}
